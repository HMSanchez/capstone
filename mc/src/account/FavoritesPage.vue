<template>
  <div>
    <div class="content" id="add">
      <h2 class="content-head is-center">My Favorites</h2>
      ...
      <div v-if="favorites.length > 0" class="pure-g">
        <div class="pure-g">
                  ...
                  <li class="pure-menu-item pure-menu-has-children pure-menu-allow-hover">
                    ...
                    <ul class="pure-menu-children items">
                      <li class="pure-menu-item">
                        <div @click="addToTopFive(index)" class="pure-menu-link">Add to Top Five</div>
                      </li>
...
export default {
  ...
  methods: {
    ...
    remove(ind) {
      ...
      var ref = this;
      var user = firebase.auth().currentUser;
      var selectedItem = this.favorites[ind];

      var songsRef = firebase
        .database()
        .ref("userObjects/" + user.uid + "/favorites/" + selectedItem.key);
      songsRef.remove();
        ...
        .then(function(snapshot) {
          ...
          if (!ref.itemInTopFive) {
            ...
            if (snapshot.numChildren() < 5) {
              ...
              ref.itemInTopFive = true;
              ref.launch_success(
                `${selectedItem.value.name} added to Top Five`
              );
            } else {
              ref.launch_error(`Top Five is set`);
            }
          } else {
            ref.launch_error(`${selectedItem.value.name} already in Top Five`);
          }
        });
    },
    ...
    addToPlaylist(ind, playlistIndex) {
      ...
      var user = firebase.auth().currentUser;
      var ref = this;
      this.songObject = this.favorites[ind].value;
